...
class FaceVerificationDemo:
    ...
    def create_sample_images(self):
        ...
        # Crear imágenes sintéticas simples para demo
        for i in range(5):
            # Crear imagen sintética (cara simulada)
            img = np.random.randint(0, 255, (112, 112, 3), dtype=np.uint8)
            
            # Añadir estructura básica de cara
            cv2.circle(img, (56, 56), 40, (200, 180, 160), -1)  # Cara
            cv2.circle(img, (45, 45), 5, (0, 0, 0), -1)         # Ojo izquierdo
            cv2.circle(img, (67, 45), 5, (0, 0, 0), -1)         # Ojo derecho
            cv2.circle(img, (56, 56), 3, (100, 50, 50), -1)     # Nariz
            cv2.ellipse(img, (56, 70), (10, 5), 0, 0, 180, (50, 50, 50), 2)  # Boca
            
            # Añadir variación por identidad
            if i < 2:  # Identidad 1
                cv2.circle(img, (56, 35), 15, (150, 100, 50), -1)  # Cabello
            elif i < 4:  # Identidad 2
                cv2.rectangle(img, (45, 30), (67, 40), (200, 150, 100), -1)  # Cabello diferente
    ...
    def demo_embedding_extraction(self, image_path: str):
        """
        Demostrar extracción de embeddings
        ...
        plt.title("Distribución del Embedding")
        plt.xlabel("Valor")
    ...
    def demo_face_verification(self, image_path1: str, image_path2: str):
        """
        Demostrar verificación facial entre dos imágenes
        
        Args:
            image_path1, image_path2: Rutas de las imágenes a comparar
        """
        ...
        # Calcular similitudes con diferentes métricas
        ...
        # Verificar identidad con diferentes umbrales
    ...
    def visualize_verification_result(self, image_path1: str, image_path2: str, 
        ...
        """Visualizar resultado de verificación"""
        
        # Cargar y alinear imágenes
        ...
        plt.title("Métricas de Similitud")
        plt.ylabel("Valor")
        ...
        plt.title("Decisión por Umbral")
        plt.xlabel("Umbral")
        plt.ylabel("Match (1) / No Match (0)")
        plt.ylim(0, 1.2)
...
"""
    ...
    def run_complete_demo(self):
        ...
        # Paso 1: Verificar o crear imágenes de muestra
        ...
        # Paso 2: Demo de alineación facial
        print(f"\n{'='*60}")
        print("🔧 DEMO: ALINEACIÓN FACIAL")
        print(f"{'='*60}")
        
        aligned_face = self.demo_face_alignment(str(image_files[0]))
        
        # Paso 3: Demo de extracción de embeddings
        print(f"\n{'='*60}")
        print("🧠 DEMO: EXTRACCIÓN DE EMBEDDINGS")
        print(f"{'='*60}")
        
        embedding = self.demo_embedding_extraction(str(image_files[0]))
        
        # Paso 4: Demo de verificación facial
        print(f"\n{'='*60}")
        print("🔍 DEMO: VERIFICACIÓN FACIAL")
        print(f"{'='*60}")
        
        # Comparar primera imagen consigo misma (debería ser match)
        print("\n🔄 Comparación 1: Imagen consigo misma (debería ser MATCH)")
        self.demo_face_verification(str(image_files[0]), str(image_files[0]))
        
        # Comparar dos imágenes diferentes (puede o no ser match)
