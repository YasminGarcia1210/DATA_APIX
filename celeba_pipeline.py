...
class CelebAProcessor:
    ...
    def download_celeba_sample(self, num_images: int = 1000):
        ...
        if not images_dir.exists():
            ...
            # Nota: Para el dataset completo, usar el enlace oficial de CelebA
            # gdown.download(sample_url, str(zip_path), quiet=False)
            
            # Crear directorio de muestra para demostración
    ...
    def align_face(self, image_path: str) -> Optional[np.ndarray]:
        ...
        try:
            ...
            # Tomar el rostro más grande (mayor área de bbox)
            face = max(faces, key=lambda x: (x.bbox[2] - x.bbox[0]) * (x.bbox[3] - x.bbox[1]))
            
            # Obtener imagen alineada (norm_crop ya está alineada)
    ...
    def extract_embedding(self, aligned_face: np.ndarray) -> Optional[np.ndarray]:
        ...
        """
        try:
            # Detectar rostro en imagen alineada
            faces = self.face_app.get(aligned_face)
            
            if len(faces) == 0:
                return None
            
            # Obtener embedding del primer rostro
    ...
    def process_images(self, 
        ...
        # Cargar identidades si están disponibles
        ...
        for img_file in tqdm(image_files, desc="Procesando imágenes"):
            try:
                ...
                # Guardar rostro alineado si se solicita
                if save_aligned:
                ...
                # Organizar embedding por identidad
...
def main():
    ...
    processor = CelebAProcessor(
        ...
        model_name="buffalo_l",  # Puedes cambiar a "arcface_r50" u otros
        device="cpu"
    )
    
    # Paso 1: Descargar/configurar CelebA
    ...
    # Paso 2: Procesar imágenes (alineación + embeddings)
    results = processor.process_images(
        images_dir=images_dir,
        max_images=100,  # Procesar solo 100 imágenes para prueba
        save_aligned=True,
